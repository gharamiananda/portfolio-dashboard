...
const ProductList: React.FC = () => {
  ...
  const {data:productData,isLoading:productLoading, isFetching}=useGetProductsQuery(queryString);
...
const columns: TableColumnsType<DataType> = [
  ...
  
  {
    ...
    <Link to={`/projects/edit-project/${item?.slug}`}>
  ...
  <Link to={`/projects/add-project?duplicate=${item?.slug}`}>Duplicate</Link>
        </div>
    </> 
  },
];
  ...
  const products=productData?.data?.Projects?.result
  const productsMeta=productData?.data?.Projects?.meta;
